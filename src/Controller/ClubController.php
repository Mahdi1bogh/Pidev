...
class ClubController extends AbstractController
{
    ...
    public function indexUser(ClubRepository $clubRepository,TerrainRepository $terrainRepository ,Request $request,PaginatorInterface $paginator): Response
    {    

        $donnees=$clubRepository->findAll();
        $donnnes=$terrainRepository->findAll();
    ...
    public function recherche(ClubRepository $clubRepository,TerrainRepository $terrainRepository,Request $request,PaginatorInterface $paginator): Response
    {  $donnnes=$terrainRepository->findAll();
        $club = $request->get('q');
        $donnees=$clubRepository->recherche($club);
        $clubs = $paginator->paginate(
            $donnees, // Requête contenant les données à paginer (ici nos articles)
            $request->query->getInt('page', 1), // Numéro de la page en cours, passé dans l'URL, 1 si aucune page
            3// Nombre de résultats par page
    ...
    public function search(Request $request , ClubRepository $cr , PaginatorInterface $paginator, TerrainRepository $terrainRepository,SerializerInterface $serializer): JsonResponse
    {
        $query = $request->get('q');
    ...
    public function sort(Request $request , ClubRepository $cr , PaginatorInterface $paginator, TerrainRepository $terrainRepository,SerializerInterface $serializer): JsonResponse
    {
        $query = $request->get('q');
        $sort = $request->get('sort', 'name'); // Default to sorting by id if no sort parameter is provided
    ...
    public function trie(ClubRepository $clubRepository,TerrainRepository $terrainRepository,Request $request,PaginatorInterface $paginator): Response
    {  $donnnes=$terrainRepository->findAll();
        $donnees=$clubRepository->Trieparclub();
        $clubs = $paginator->paginate(
            $donnees, // Requête contenant les données à paginer (ici nos articles)
            $request->query->getInt('page', 1), // Numéro de la page en cours, passé dans l'URL, 1 si aucune page
            3// Nombre de résultats par page
